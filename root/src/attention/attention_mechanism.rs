...
/// Computes scaled dot-product and multi-head attention.

/// Functional: scaled_dot_product_attention
/// Computes the scaled dot-product attention for a set of queries, keys, and values.
///
/// Parameters:
///   - `query`: The Q matrix (`Vec<Vec<f64>`) representing the query vectors.
///   - `key`: The K matrix (`Vec<Vec<f64>`) representing the key vectors.
///   - `value`: The V matrix (`Vec<Vec<f64>`) representing the value vectors.
///
/// Return:
///   A 2D vector (`Vec<Vec<f64>`) representing the attention-weighted output for each query.
pub fn scaled_dot_product_attention(
	...
	assert!(
			!query.is_empty() && !key.is_empty() && !value.is_empty(),
	...
	for row in &mut qk_transpose {
			let max_val = row.iter().cloned().fold(f64::MIN, f64::max);

			let sum_exp: f64 = row.iter().map(|val| (val - max_val).exp()).sum();
			for val in row.iter_mut() {
...
///
/// Return:
///   A 2D vector (`Vec<Vec<f64>`) representing the concatenated and projected multi-head attention output.
